...
def bus_number_assign(bus_trip_assign, trip_df):
    """
      ...
      Parameters:
      ____________
      :param bus_trip_assign: list of list bus trip assignment
      :param trip_df: DataFrame trip schedule
...
def duration_bus_spent(data_charging, trip_data, num_bus):
    ...
    Parameters:
    ____________
    :param data_charging: DataFrame charging schedule
    :param trip_data: DataFrame trip schedule
    :param num_bus: int number of buses
    ...
    for i in range(len(bus_ids)):
        ...
        try:
            duration_df.loc[i, 'trip_duration'] = \
                trip_duration[trip_duration.bus_number == bus_ids[i]].duration_in_min.values[0]
            duration_df.loc[i, 'charge_duration'] = \
                charging_duration[charging_duration.bus_number == bus_ids[i]].duration_in_min.values[0]
        except IndexError:
...
def plot_gantt_chart(network, folder_path):
    """
    ...
    for scenario in scenarios:
        ...
        charging_solar = pd.read_csv(
            ...
            f'{scenario}_scenario_actual.csv')
        ...
        # date of today (get 25th July, 2024's date)
        day = date(2024, 7, 25)
        # day = date.today()
        # next_day
        ...
        trip_schedule.Start_Time = np.where(trip_schedule.Start_Day == 1,
                                            next_day_str + " " + trip_schedule.Start_Time,
                                            day_str + " " + trip_schedule.Start_Time)

        trip_schedule.End_Time = np.where(trip_schedule.End_Day == 1,
                                          next_day_str + " " + trip_schedule.End_Time,
                                          day_str + " " + trip_schedule.End_Time)
        ...
        trip_schedule["duration_in_min"] = (trip_schedule["End_Time"] - trip_schedule[
            "Start_Time"]).dt.total_seconds() / 60

        trip_schedule["bus_number"] = 0

        # make all the start day and end day as 1
        trip_schedule.Start_Day = 1
        trip_schedule.End_Day = 1

        # show the entire dataframe in console
        pd.set_option('display.max_rows', None)
        print(trip_schedule)
        # save the trip_schedule as csv
        trip_schedule.to_csv(f'{folder_path}/{network}/{number_of_scenario}_scenario/trip_schedule_trial.csv',
                             index=False)
        ...
        charging_schedule["charging_start_time"] = pd.to_datetime(charging_schedule["charging_start_time"],
                                                                  format='%d-%m-%Y %H:%M:%S').copy()
        charging_schedule["charging_end_time"] = pd.to_datetime(charging_schedule["charging_end_time"],
                                                                format='%d-%m-%Y %H:%M:%S').copy()
        charging_schedule["duration_in_min"] = (charging_schedule["charging_end_time"] - charging_schedule[
            "charging_start_time"]).dt.total_seconds() / 60
        ...
        charging_solar_schedule["charging_start_time"] = pd.to_datetime(charging_solar_schedule["charging_start_time"],
                                                                        format='%d-%m-%Y %H:%M:%S').copy()
        charging_solar_schedule["charging_end_time"] = pd.to_datetime(charging_solar_schedule["charging_end_time"],
                                                                      format='%d-%m-%Y %H:%M:%S').copy()
        charging_solar_schedule["duration_in_min"] = (charging_solar_schedule["charging_end_time"] -
                                                      charging_solar_schedule[
                                                          "charging_start_time"]).dt.total_seconds() / 60
        ...
        start_time_charging = pd.to_datetime(charging_schedule_new.charging_start_time.min().strftime("%d-%m-%y %H"),
                                             format='%d-%m-%y %H')
        end_time_str = charging_schedule_new["charging_end_time"].max().strftime("%d-%m-%y ")
        print(start_time_charging)
        print(end_time_str)
        hour = int(charging_schedule_new["charging_end_time"].max().strftime("%H"))
        print(hour)
        if hour == 24:
            hour = 1
        ...
        hour = int(trip_schedule["End_Time"].max().strftime("%H"))
        if hour == 24:
            hour = 1
            end_time_str = str(next_day.day) + end_time_str[2:]
        elif hour == 23:
            hour = 0
            end_time_str = str(next_day.day) + end_time_str[2:]
        end_time_trip = pd.to_datetime(end_time_str + f"{hour:02d}", format='%d-%m-%y %H')

        print(start_time_trip)
        print(end_time_trip)

        start_time = min(start_time_trip, start_time_charging)
        # print(start_time)
        # make start_time as 2024-07-25 18:00:00 for the Durham and Canberra network
        # start_time = pd.to_datetime("2024-07-25 18:00:00", format='%Y-%m-%d %H:%M:%S')
        # print(start_time)
        ...
        charging_schedule_new["initial_time"] = (
                                                        charging_schedule_new.charging_start_time - start_time).dt.total_seconds() / 60
        ...
        ax.barh(charging_schedule_new_solar['bus_number'] * 3, charging_schedule_new_solar.duration_in_min,
                ...
                height=1.2)
        # for both
        charging_schedule_new_solar = charging_schedule_new[charging_schedule_new.grid_or_solar == 'both']
        ax.barh(charging_schedule_new_solar['bus_number'] * 3, charging_schedule_new_solar.duration_in_min,
                left=charging_schedule_new_solar.initial_time, color='purple', label='Grid + Solar Energy',
                edgecolor='purple',
                height=1.2)
        ...
        xtick_labels = ['00', '01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12', '13', '14', '15',
        ...
        if network == 'Durham_2.1k':
            may_to_october = [i for i in range(18, 44)]
        else:
            may_to_october = []

        if scenario in may_to_october:
            rectangle_start_peak1 = pd.to_datetime(day.strftime("%d-%m-%y") + ' ' + '11', format='%d-%m-%y %H')
            rectangle_end_peak1 = pd.to_datetime(day.strftime("%d-%m-%y") + " 17", format='%d-%m-%y %H')
            ...
            ax.add_patch(plt.Rectangle((start_time_peak1, 0), end_time_peak1 - start_time_peak1,
                                       ...
                                       edgecolor='grey'))
            # rectangle_start_peak2 = pd.to_datetime(next_day.strftime("%d-%m-%y") + ' ' + '11', format='%d-%m-%y %H')
            # rectangle_end_peak2 = pd.to_datetime(next_day.strftime("%d-%m-%y") + " 17", format='%d-%m-%y %H')
            ...
            #                            3 * len(trip_schedule.bus_number.unique()), fill=True, color='grey', alpha=0.3,
            #                            edgecolor='grey'))
            rectangle_start_off_peak1 = pd.to_datetime(day.strftime("%d-%m-%y") + ' ' + '7', format='%d-%m-%y %H')
            rectangle_end_off_peak1 = pd.to_datetime(day.strftime("%d-%m-%y") + " 11", format='%d-%m-%y %H')
            rectangle_start_off_peak2 = pd.to_datetime(day.strftime("%d-%m-%y") + ' ' + '17', format='%d-%m-%y %H')
            rectangle_end_off_peak2 = pd.to_datetime(day.strftime("%d-%m-%y") + " 19", format='%d-%m-%y %H')
            # rectangle_start_off_peak3 = pd.to_datetime(next_day.strftime("%d-%m-%y") + ' ' + '17', format='%d-%m-%y %H')
            # rectangle_end_off_peak3 = pd.to_datetime(next_day.strftime("%d-%m-%y") + " 19", format='%d-%m-%y %H')
            ...
            if network.startswith('Canberra'):
                rectangle_start_peak = pd.to_datetime(day.strftime("%d-%m-%y") + ' ' + '7', format='%d-%m-%y %H')
                rectangle_end_peak = pd.to_datetime(day.strftime("%d-%m-%y") + " 17", format='%d-%m-%y %H')
                rectangle_start_shoulder1 = pd.to_datetime(day.strftime("%d-%m-%y") + ' ' + '17', format='%d-%m-%y %H')
                ...
                ax.add_patch(plt.Rectangle((start_off_peak, 0), end_off_peak - start_off_peak,
                                           3 * len(trip_schedule.bus_number.unique()), fill=True, color='pink',
                                           alpha=0.3,
                                           edgecolor='pink'))
                ...
                ax.add_patch(plt.Rectangle((start_peak, 0), end_peak - start_peak,
                                           3 * len(trip_schedule.bus_number.unique()), fill=True, color='grey',
                                           alpha=0.3,
                                           edgecolor='grey'))
                ...
                start_time_off_peak2 = (rectangle_start_off_peak2 - start_time).total_seconds() / 60
                end_time_off_peak2 = (rectangle_end_off_peak2 - start_time).total_seconds() / 60
                # print(start_time_off_peak2, end_time_off_peak2)
                ax.add_patch(plt.Rectangle((start_time_off_peak2, 0), end_time_off_peak2 - start_time_off_peak2,
                                           3 * len(trip_schedule.bus_number.unique()), fill=True, color='grey',
                                           alpha=0.3,
                                           edgecolor='grey'))
        ...
        legend_elements = [Patch(facecolor='#87CEEB', edgecolor='#4C87B9', label='Trip'),
                           Patch(facecolor='red', edgecolor='red', label='Grid'),
                           Patch(facecolor='green', edgecolor='green', label='Solar'),
                           Patch(facecolor='purple', edgecolor='purple', label='Grid + Solar'),
                           Patch(facecolor='grey', edgecolor='grey', label='Peak Hours'),
                           Patch(facecolor='pink', edgecolor='pink', label='Mid Peak Hours'),
                           Patch(facecolor='white', edgecolor='black', label='Off Peak Hours')]
        ax.legend(handles=legend_elements, fontsize=45,
                  loc='upper right')  # For Durham, keep the fontsize as 35, and for Canberra keep it as 45
