...
def analyze_master_variables(network, folder_path):
    """
    ...
    for index, scenarios in enumerate(list_scenarios):
        ...
        if renewables:
            ...
            data_master['variable_type'] = data_master['variable_type'].map(
                {'z': 'grid_capacity', 's': 'battery_capacity',
                 'a': 'panel_area'})
                ...
                data_master_scenarios = data_master_scenarios._append(
                    {'scenario': scenarios, 'location': location,
                     ...
                     'battery_capacity': data_master[(data_master['location'] == location) &
                                                     (data_master['variable_type'] == 'battery_capacity')][
                         'value'].values[
                         0],
            ...
            for location in locations:
                data_master_scenarios = data_master_scenarios._append(
                    {'scenario': scenarios, 'location': location,
                     'grid_capacity': data_master[(data_master['location'] == location) &
                                                  (data_master['variable_type'] == 'grid_capacity')]['value'].values[
                         0]},
                    ignore_index=True)
    ...
    for scenarios in list_scenarios:
        ...
        for location in not_present_locations:
            data_master_scenarios = data_master_scenarios._append(
                {'scenario': scenarios, 'location': location, 'grid_capacity': 0, 'battery_capacity': 0,
                 'panel_area': 0},
                ignore_index=True)
    ...
    #     52: ''
    # }
    dict_units = {'grid_capacity': 'Grid Capacity (kW)', 'battery_capacity': 'Battery Capacity (kWh)',
                  'panel_area': 'Panel Area (m$^2$)'}
    # for grid capacity
            ...
            for scenarios in list_scenarios:
                try:
                    if data_master_scenarios[(data_master_scenarios['scenario'] == scenarios) &
                                             (data_master_scenarios['location'] == location)][master].values[
                        0] >= 1e-04:
                        break
                    else:
                        count += 1
    ...
    dict_fig_size = {'grid_capacity': (30, 5), 'battery_capacity': (30, 5),
                     'panel_area': (30, 5)}  # original size (30, 12)
    # dict_fig_size = {'grid_capacity': (15, 30), 'battery_capacity': (15, 30), 'panel_area': (15, 30)}
    dict_bar_width = {'grid_capacity': 0.15, 'battery_capacity': 0.15, 'panel_area': 0.15}  # 0.2 or 0.22 previously
    # 0.10 (Durham), 0.08 (Canberra) (Grid capacity plot width)
    dict_font_size = {'grid_capacity': 22, 'battery_capacity': 22, 'panel_area': 22}  # original size is 32
    dict_label_size = {'grid_capacity': 22, 'battery_capacity': 22, 'panel_area': 22}  # original_size is 40
    ...
    for master in list_master:
        ...
        # bar plot with horizontal axis ends

        # # bar plot with axis transposed
        # for i, j in enumerate(list_scenarios):
        #     data = data_master_scenarios_[data_master_scenarios_['scenario'] == j][master]
        #     ax.barh(indices + i * bar_width, data, bar_width, label=f'{j} scenario', color=color_dict[j],
        #             edgecolor='#323232')
        # # flip the x-axis and y-axis labels
        # # make the yticklabels same as the locations
        # ax.set_yticks(indices)
        # ax.set_yticklabels([locations[i] for i in range(len(indices))])
        # # ax.set_yticklabels([location for location in locations])
        ...
        # ax.set_ylabel('Charging Location', fontsize=dict_label_size[master])
        # ax.set_xlabel(dict_units[master], fontsize=dict_label_size[master])

        # # make a line plot
        # for i, j in enumerate(list_scenarios):
        #     data = data_master_scenarios_[data_master_scenarios_['scenario'] == j][master]
        #     ax.plot(indices, data, label=f'{j} scenario', color=color_dict[j], marker=None)
        ...
        # Display the legend
        # place the legend so that it does not overlap with the bars
        ax.legend()
        # increase the font size of legend
        # plt.legend(fontsize=dict_font_size[master], loc='upper right')
        if network == 'Canberra_3.91k' and master == 'grid_capacity':  # shift the legend slightly to the left so that it does not overlap with the bars
            plt.legend(fontsize=15, loc='upper right')
        else:
            plt.legend(fontsize=15, loc='upper right')
        # save the plot
        plt.savefig(f'{folder_path}/{network}/visualization/master_variables_{master}_{network}.pdf',
                    bbox_inches='tight')
        # Show the plot
        # plt.show()

    # Total contracted grid cost
    Unit_battery_price = np.asarray([parameters.Unit_battery_price[0]], dtype=np.float64)
    Unit_capacity_cost = np.asarray([parameters.Unit_capacity_cost[0]], dtype=np.float64)
    Unit_panel_cost = np.asarray([parameters.Unit_panel_cost[0]], dtype=np.float64)
    for scenarios in list_scenarios:
        total_grid_cost = data_master_scenarios[data_master_scenarios['scenario'] == scenarios]['grid_capacity'].sum() * \
                          Unit_capacity_cost[0]
        total_battery_cost = data_master_scenarios[data_master_scenarios['scenario'] == scenarios][
                                 'battery_capacity'].sum() * Unit_battery_price[0]
        total_panel_cost = data_master_scenarios[data_master_scenarios['scenario'] == scenarios]['panel_area'].sum() * \
                           Unit_panel_cost[0]
